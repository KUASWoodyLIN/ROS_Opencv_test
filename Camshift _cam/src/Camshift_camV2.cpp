...
bool showHist = true;
Point origin;
Rect selection = Rect (100,100,50,50);
...
int main( int argc, char** argv )
{
    ...
    Mat hsv_input,mask_input,hue_input;
    bool paused = false;
    ...
    for(;;)
    {
	    ...
	    cvtColor(img, hsv_input, COLOR_BGR2HSV);
            if( trackObject )
            {
		...
		/**********/
		inRange(hsv_input, Scalar(0, smin, MIN(_vmin,_vmax)),
                        Scalar(180, 256, MAX(_vmin, _vmax)), mask_input);
                hue_input.create(hsv_input.size(), hsv_input.depth());
                mixChannels(&hsv_input, 1, &hue_input, 1, ch, 1);
		/**********/
                if( trackObject < 0 )
                {
		    //roi为选中区域的矩阵，maskroi为0-1矩阵
                    Mat roi(hue_input, selection), maskroi(mask_input, selection);
